# latent_working_memory（20260910 13:30:43 UTC+08:00）

最后修订时间：20260910 13:30:43 UTC+08:00
...
数据准备代码位于 `src/latent_working_memory/data_preparation/`。共享来源池固定文档与重复簇的 train/dev/test 划分；semantic/random 的 AE 与 LM 分别抽取 X，LM 再选择紧邻的 Y。四类任务各自按 X 长度区间均衡补足模型判定后的配额，文本粒度作为统计属性。Qwen3.8-27B 判定最终 X/Y 质量，独立抽查检查完成数据。当前规则与模块职责见 [4k 独立任务数据准备](notes/v1/20260910_4k_independent_data_preparation.md)。

此前完成的两套逐样本配对数据，每套包含 226,022／11,610／11,685 个 train/dev/test 样本，作为历史产物保留，见 [配对数据构造记录](notes/v1/20260909_paired_boundary_pretraining_data.md)。20260909 已使用 Qwen 完成 semantic/random 各 100 条数据的构造和逐条复核；本次更新实现新的 4k 长度与独立任务规则。

`configs/v1/pretrain_a800.json` 保存模型与训练采样设置，`configs/data_preparation/fineweb.json` 保存准备配方。`samples_per_task=[100000,2000,2000]` 表示每个版本的 train/dev/test 中，AE 与 LM 分别达到对应数量；AE 的 X、LM 的 X 与 Y 分别为 32–4096 tokens；LM 满足 `0.3 <= |X| / (|X| + |Y|) <= 0.7`。`length_bounds` 默认定义 32–64、65–128、129–256、257–512、513–1024、1025–2048、2049–4096 tokens 七档，四类任务每档的最终样本数最多相差一条。评分服务就绪后执行：
...
单条判定发生输出截断或格式错误时，客户端重试一次，并在评分缓存的 `failures` 中保存原始错误回复。连续两次失败记为技术状态 `error`，主流程记录该状态并继续补足有效样本配额；独立抽查保留为未完成。技术错误与模型的 `reject`、`uncertain` 分开统计，连接和服务异常仍会中止任务并保留现场。
...
`--stage sources` 建立共享来源池；`--stage semantic` 与 `--stage random` 分别执行构造、模型判定和检查，后者读取已完成 semantic 的配方和均衡分布并执行交叉校验。根目录保存来源登记和 `comparison.json`，各版本保存三个 split、入选原文、候选判定、`audit.json` 与完成记录 `preparation.json`。质量抽查通过独立模块 `latent_working_memory.data_preparation.inspection` 的 `sample`、`judge`、`summarize` 执行，支持模型及人工复核。正式训练验证的数据入口为 `semantic/`。
