# 20260909_可增长 Latent Working Memory 框架 v1（11:40:55 UTC+08:00）
...
自然边界确定的 FineWeb 原文片段为 $X$，其 token 数为 $L_X$；同一文档紧接其后的连续原文为 $Y$，长度为 $L_Y$。候选数量压缩率为 $\mathcal R=\{2,4,8\}$，各比例对应容量：
...
自重建（AE）恢复已经写入的 $X$；后续语言建模（LM）预测 $Y$。两项使用同一个 $M_X^{(K_X)}$：

$$
\mathcal L_{\mathrm{AE}}
=\ell_\eta(M_X^{(K_X)},q_{\mathrm{AE}},X),\qquad
...
=\mathbb E_{(X,Y),\,K_X\sim p(K\mid L_X,L_Y)}
\left[
\lambda_{\mathrm{AE}}\mathcal L_{\mathrm{AE}}
+\lambda_{\mathrm{LM}}\mathcal L_{\mathrm{LM}}
\right].
$$

两项权重为运行配置中的正数。$X$ 构成写入输入，$Y$ 作为未来目标交给读取路径。目标直接取自原文，训练保留完整序列及 EOS。生成评价的输出预算按任务固定。
...
每次先采样源文档，再采样粒度与片段 $(X,Y)$，最后采样有效容量 $K_X$。每个样本从空记忆开始，以一个容量完成一次写入，联合计算 AE 与 LM 损失。再次访问同一片段时可以重新采样容量，预训练产物为动态阶段直接继承的 $\eta$。
...
每个 FineWeb 预训练 episode 对应一份 $(X,Y)$：`input_ids` 为 $X$，`write_ends=[L_X]`，AE 与 continuation 两个 reads 共享 `prefix_end=L_X`。$Y$ 保存在 continuation 的参考目标中，来源记录标明 $X,Y$ 在同一原文中的位置。$K_X$ 在训练访问时采样并写入运行日志。
...
1. 保留原文、文档 ID、URL、抓取来源及日期，依据稳定来源标识确定 train/dev/test。同源重复文档及其全部派生片段归入同一 split。
2. 识别段落和句子边界，保留原文字符位置、顺序及实际段落结构。满足上下文预算的完整句子构成片段的基本单位。
3. 从同一文档构造多粒度候选，每个粒度优先覆盖不同自然长度；训练依次按长度区间、文档、粒度与片段采样。
4. 合法片段 $X$ 直接具有 AE 目标，存在合格紧邻原文时附加 LM 目标 $Y$。续写终点从预算内的完整句末选择。完整目标及特殊 tokens 一起参与容量与上下文预算计算；每次参数更新按各任务有效样本数归一化损失。
5. 将选定片段编译为第 8.2 节的统一 episode。来源记录保存原文位置、粒度、切分方式、句子索引及相关标注来源。

质量筛选覆盖语言置信度、模板页面、结束标点、截断迹象、引用列表、代码及重复内容。句界检测解析引号内的句子，原文字符位置保持一致；合格的连续句段用于构造输入及紧邻续写。来源 URL、重复文档与跨来源相同长片段分别去重；近重复文档簇共享数据划分。准备记录包含筛选规则、独立随机抽查、分层抽查、长度分位数与最终来源配额。

数据准备代码统一位于 `src/latent_working_memory/data_preparation/`。NLL 采用连续原文窗口独立计算并按目标 token 数汇总，作为流畅度诊断信号。语义质检模型按统一标准返回保留、排除或待复核判定，局部缺陷记录为原文区间；评分缓存保留模型、提示词和判定结果。入口与数据契约见 [数据准备重构记录](v1/20260909_data_preparation_refactor.md)。
...
切分对照采用相同源文档和计算预算，并进一步匹配输入长度、容量与重复曝光，分别衡量语义边界、长度分布和额外标注成本的作用。
...
| 预训练目标 | AE 与 LM 联合，共享当前容量的记忆 | 分别使用 AE 或 LM |
...
预训练链路已实现空状态、统一零初值、外部首批容量、自然单元完整前向、变长 batch、FineWeb 多粒度 adapter、AE/LM 联合目标、多压缩率采样、训练恢复和独立文档多容量评估。本地 49 项测试与服务器 43 项 v1 测试通过。FineWeb `sample-10BT` 已下载到服务器，Llama-2-7B-Chat 单卡训练、保存恢复与多容量评估已跑通。扩大数据实验已准备 10,000 篇训练文档及各 512 篇 dev/test 文档，训练集包含 126,278 个样本对；两组学习率预训练各完成 2000 步；dev 选择 3e-5 第 2000 步 checkpoint。512 文档独立 test 的 AE/LM 正确记忆 NLL 为 2.2532/2.3914，相对空记忆收益为 0.1409/0.1242；64 文档、三个容量的自由重建完整匹配为 0/192，平均归一化 token 编辑距离为 0.9391。正确记忆已辅助条件预测，自由重建能力仍弱。结果详见 [扩大数据预训练记录](v1/20260908_fineweb_generalization_pretraining_record.md)。后续工程任务为 MSC 对话调度、动态训练、容量学习及完整系统对照。
...
代码位于 `src/latent_working_memory/v1/`，产物位于 `data/v1/`、`checkpoints/v1/`、`artifacts/v1/<run_id>/`。实现采用 PyTorch、Transformers、PEFT 和 `uv` 管理的环境。SwanLab 记录训练与独立评估指标、分层曲线和重建样例，实验 ID 与可视化选项独立于模型配置保存。GPU 实验限定物理 GPU 0、1；启动命令显式设置 `CUDA_VISIBLE_DEVICES=0`、`1` 或 `0,1`。
