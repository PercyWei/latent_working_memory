# 20260910_真实公开数据与记忆训练路线（15:45:45 UTC+08:00）

创建时间：20260907 22:31:33 UTC+08:00

最后修订时间：20260910 15:45:45 UTC+08:00
...
AE 独立抽取原文 X，写入并重建 X；LM 独立抽取 X 及其紧邻后缀 Y，写入 X，监督 Y 的全部正文 tokens 与 EOS。两项各自构造成单任务 episode，损失按各任务有效样本数归一化后加权。

当前构造 semantic 与 random 两种边界版本，按任务及 X 长度档均衡补足模型判定后的配额。长度、来源隔离、评分和独立抽查规则统一见 [预训练数据构建](v1/20260910_pretraining_data_construction.md)。容量由训练采样器按实际 X 长度、压缩率及完整上下文预算选择。
...
每轮完成后封闭写入事件，在上下文预算内作为一个变长单元完整处理；超预算事件沿原文段落或句界拆为合法单元，顺序提交。更新器沿自身产生的记忆连续运行，session 边界沿用已有记忆。
