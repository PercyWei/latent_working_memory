# 20260908_FineWeb 预训练链路与小样本试验记录（21:29:25 UTC+08:00）

创建时间：20260908 20:20:50 UTC+08:00

最后修订时间：20260908 21:29:25 UTC+08:00
...
数据保存在服务器项目的 `data/v1/fineweb-pilot-20260908/`。本地准备统计见 [`preparation.json`](../../artifacts/v1/fineweb-pilot-20260908/preparation.json)。
...
服务器 checkpoint 位于 `artifacts/v1/pretrain-smoke-20260908/checkpoints/`；本地日志与评估结果位于 [`artifacts/v1/pretrain-smoke-20260908/`](../../artifacts/v1/pretrain-smoke-20260908/)。

额外变长 batch 验证将 1-token 与 1,024-token 的真实输入放入同一批次，分别分配 1 与 512 个记忆位置，完成 AE/LM 前向、反向与参数更新。写入投影、记忆更新器、读取投影与读取 LoRA 的梯度均非零且有限，峰值显存约 14.51 GiB。记录见 [`stress-batch.json`](../../artifacts/v1/pretrain-server-validation/stress-batch.json)。

自由生成由 Hugging Face 根据持续扩展的 attention mask 计算位置编号；回归测试验证 KV cache 逐步生成的 logits 与完整前缀读取一致。该检查修正了首次评估中位置编号固定的问题。以下质量结果使用修正后的评估路径，原始结果保存在运行目录的 `superseded-generation/`。
...
修正前后的 dev NLL 完全一致；修正改变了自由生成时的缓存位置推进。完整结果见本地 [`artifacts/v1/pretrain-overfit-20260908/`](../../artifacts/v1/pretrain-overfit-20260908/)，训练前后面板分别位于 `fixed-panel-step-000000/` 与 `fixed-panel-step-000100/`，独立 dev 的 step 50、100 汇总位于运行目录根部。
...
服务器数据目录统一为 `data/raw/HuggingFaceFW-fineweb/sample-10BT/`。配置、预处理契约与已有 checkpoint 已完成配套迁移，模型参数、优化器和随机状态保持一致。迁移前文件保存在项目的 `artifacts/v1/fineweb-layout-swanlab-20260908/backups/`。
...
在线记录通过 `--swanlab-mode online` 开启，离线记录使用 `offline`，默认 `disabled`。训练在同一输出目录恢复时读取 `swanlab.json` 并续接原实验；独立评估可用 `--swanlab-run-id` 指定该实验。开启记录的 tiny Llama 训练及恢复与连续运行结果一致。验收结果见 [`verification.json`](../../artifacts/v1/fineweb-layout-swanlab-20260908/verification.json)。
