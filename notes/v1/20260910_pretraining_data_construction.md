# 20260910_预训练数据构建策略与实现（16:30:38 UTC+08:00）

创建时间：20260910 15:44:35 UTC+08:00
最后修订时间：20260910 16:30:38 UTC+08:00
...
评分通过独立 HTTP 服务直连调用，temperature 为 0，关闭 thinking，返回 `decision` 和不超过 240 字符的 `reason`。缓存绑定完整 X/Y、模型、地址和评分协议，逐条保存。输出截断或判定格式错误时重试一次，仍失败则记为 `error` 并跳过候选；连接或服务错误直接中止阶段。完成目录不覆盖，失败阶段重建使用新目录并复用评分缓存。
