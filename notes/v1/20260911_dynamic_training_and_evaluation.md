...
SwanLab 使用 `latent-working-memory-v1`，同系列训练、评估与比较共享显式 group；`job_type` 分别为 `train`、`evaluate`、`compare`，`study:*` 标签通过启动参数传入。训练 run 的评估图统一位于 `evaluation/dev/*`：NLL、EM、F1、触顶率各一张历史折线图，横轴为 optimizer step，五条曲线对应五种对照条件。`evaluation/dev/by-capacity/*` 比较各记忆容量的 memory 指标，`evaluation/dev/paired/*` 展示 memory 相对各对照的 NLL、EM 和 F1 差值。每次评估更新历史曲线，生成指标使用实际生成评估点。完整数值保存在本地 JSON/JSONL，表格与问答样例位于 `tables/*`、`examples/*`。独立评估与跨运行比较的汇总图同样位于 `evaluation/*`。测试设置 `--swanlab-mode disabled`，正式实验设置 `online`。
