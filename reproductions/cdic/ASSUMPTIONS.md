# C-DIC 假设清单（20260908 19:00:56 CST）

创建时间：20260904 16:19:08 CST（UTC+08:00）

最后修订时间：20260908 19:00:56 CST（UTC+08:00）
...
| Retrieved state 顺序 | Retrieved states 表示为无序集合 | 按衰减后得分降序排列，同分时保持 memory 写入顺序 | 实现假设 |
| 相似度同分 | 未说明 | 选择 memory 中更早的 state 作为 argmax | 实现假设 |
| Recency 重置 | `Delta t` 表示距上次 retrieval 的 turn 数 | 所有 retrieved states 均重置，包括 off-topic fallback；新写入 state 从当前 turn 开始计时 | 实现假设 |
...
| EOS 处理 | C-DIC 未说明；旧 ICAE 实现自行固定 EOS ID | 直接使用基础模型 tokenizer 的 `eos_token_id`；仅新增独立的 memory、control 与 padding tokens | 本地现代接口测试已覆盖，待 GPU 复验 |
...
| LoRA 细节 | 仅说明训练 compressor 与 compression tokens | 按公开 ICAE v1 checkpoint 固定 LoRA rank 128；离线转换为仅含 LoRA 与 memory/control token embeddings 的 canonical state dict | canonical checkpoint 已在 A800 转换并校验，待现代模型 GPU strict load |
...
| Gradient checkpointing | 旧 ICAE 自定义 decoder branch 需要手工转发 LoRA 开关 | 现代实现直接使用 Transformers 与 PEFT 的 gradient-checkpointing 接口，不保留旧 `enable_lora` 分支 | 本地 tiny-model backward 已覆盖，待 A800 gradient audit |
