...
状态：R1 核心机制与 inference adapter 已实现，待 GPU 验证
...
| Query representation | 使用 `psi(f_comp(q_t, C))` | 使用相同 compression-token bank 压缩 query | 已实现，待 GPU 验证 |
...
| LoRA 细节 | 仅说明训练 compressor 与 compression tokens | 加载前从 ICAE checkpoint 推断 rank 和参数形状 | 待 checkpoint 验证 |
