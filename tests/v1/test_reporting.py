...
def test_report_axes_and_missing_values():
    ...
    assert charts["report/all/ae/nll"].options["xAxis"][0]["data"] == ["memory", "no_memory"]
    chart = charts["report/length_up_to/ae/nll"].options
...
def test_comparison_uses_explicit_labels_and_separate_metrics():
    ...
    opts = charts["comparison/ae/nll"].options
    assert opts["xAxis"][0]["data"] == ["A", "B"]
    assert opts["series"][0]["data"] == [1.0, 2.0]
    assert charts["comparison/ae/bleu_4"].options["series"][0]["data"] == [4.0, 5.0]
