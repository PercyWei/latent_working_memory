...
def test_live_training_and_saved_replay_emit_identical_records(
    ...
    data = tmp_path / "data"
    prepare_fineweb(
        parquet_source(preparation_records), tokenizer, config, data, preparation_recipe
    )
    live = RecordingRun()
    configs = []
    ...
    training.run_pretraining(
        config,
        epoch_selection(data, config, {"dev": "semantic"}),
        output,
        torch.device("cpu"),
        epochs=2,
        stop_after_steps=2,
...
def test_replay_preserves_inherited_token_counters(tmp_path):
    (tmp_path / "config.json").write_text(
        json.dumps({"batch_size": 2, "gradient_accumulation_steps": 2})
    )
    ...
    (tmp_path / "resources-from-005000-test.json").write_text(
        json.dumps(
            {
                "completed_steps": 5002,
                "cumulative_input_tokens": 1000,
                "cumulative_target_tokens": 2000,
            }
        )
    )
    (tmp_path / "semantic").mkdir()
    (tmp_path / "semantic/dev-step-005002.json").write_text(
        json.dumps({"training_input_tokens": 1000})
    )
