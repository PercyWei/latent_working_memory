...
def test_random_samples_obey_actual_length_interval_and_raw_continuity(
    ...
    sampler = RandomSpans(record, tokenizer, tiny_config)
...
def test_random_byte_slices_handle_whitespace_and_subwords(tiny_config, preparation_records):
...
def test_semantic_lm_writes_only_internal_prefix(tiny_config, tokenizer, preparation_records):
    for episode in document_episodes(preparation_records[0], tokenizer, tiny_config):
        if episode.reads[0].task == "continuation":
            p = episode.sources[0].provenance
...
def test_continuation_has_no_eight_sentence_cap(tiny_config, tokenizer, preparation_records):
    record = dict(
